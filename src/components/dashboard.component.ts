...
@Component({
  ...
  template: `
    <div class="h-full flex flex-col space-y-6 overflow-y-auto pb-6 relative animate-fadeIn">
      ...
      <!-- WHATSAPP AUTOMATION BANNER - PREMIUM DISPATCH DESIGN -->
      @if (juristService.readyAlertsCount() > 0) {
        <div class="bg-gradient-to-br from-green-950/40 via-zinc-900 to-black border border-green-500/40 rounded-[2rem] p-8 flex flex-col lg:flex-row items-center justify-between gap-8 animate-slideDown mb-8 ring-1 ring-green-400/20 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8)] relative overflow-hidden group">
           <div class="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(34,197,94,0.1),transparent_70%)] opacity-50"></div>
           
           <div class="flex items-center gap-6 relative z-10 text-center lg:text-left flex-col lg:flex-row">
             <div class="bg-green-500 p-4 rounded-3xl text-black shadow-[0_0_25px_rgba(34,197,94,0.4)] transition-all">
              ...
              <div>
                <h3 class="text-white font-black text-2xl mb-2 leading-none tracking-tight">Centru de Dispatch WhatsApp</h3>
                <p class="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
                  Identificate <span class="text-green-400 font-black">{{ juristService.readyAlertsCount() }}</span> reamintiri automate pregătite pentru clienți.
                  <span class="block mt-2 text-[10px] text-green-500/60 font-bold uppercase tracking-widest bg-green-500/5 px-2 py-1 rounded inline-block border border-green-500/10">Sincronizare calup activă: Termene 24h</span>
                </p>
              </div>
           </div>
           
           <div class="flex flex-col sm:flex-row gap-4 w-full lg:w-auto relative z-10 shrink-0">
             <button (click)="sendAllAlerts()" class="bg-green-600 hover:bg-white hover:text-black text-white px-10 py-5 rounded-2xl text-base font-black transition-all shadow-2xl active:scale-95 flex items-center justify-center gap-3 group/btn">
               TRIMITE TOATE ACUM
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    // Keep alerts fresh if the user stays on dashboard
    this._alertInterval = setInterval(() => {
       // Just accessing the signal computed will refresh if events updated, 
       // but we forces a check of the window logic
    }, 60000); // 1 min check
  }
