  ...
  template: `
    <div class="h-full flex flex-col space-y-6 overflow-y-auto pb-6 relative animate-fadeIn">
      ...
      @if (juristService.readyAlertsCount() > 0) {
        <div class="bg-green-900/40 border border-green-500 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 animate-fadeIn mb-6 ring-2 ring-green-500/20 shadow-[0_0_20px_rgba(34,197,94,0.1)]">
           <div class="flex items-center gap-3">
             <div class="bg-green-500 p-2 rounded-lg text-black animate-pulse">
             ...
             <div>
               <h3 class="text-white font-bold text-sm">Alerte WhatsApp Urgent (Termen 24h)</h3>
               <p class="text-gray-100 text-xs text-balance">Aveți <span class="text-green-400 font-bold">{{ juristService.readyAlertsCount() }}</span> evenimente scadente. Trimiteți acum informările către clienți.</p>
             </div>
           </div>
           <div class="flex gap-2">
             <button (click)="sendAllAlerts()" class="bg-green-600 hover:bg-white hover:text-black text-white px-5 py-2.5 rounded-lg text-sm font-bold transition-all shadow-lg active:scale-95 whitespace-nowrap">
               Trimite Toate Alertele Scadente
             </button>
           </div>
        </div>
      }
