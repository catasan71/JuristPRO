      ...
      @if (juristService.readyAlertsCount() > 0) {
        <div class="bg-gray-900 border border-green-600/30 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 animate-slideDown mb-8 shadow-xl transition-all">
           <div class="flex items-center gap-5">
             ...
             <div>
               <h3 class="text-white font-bold text-lg">Dispatch WhatsApp</h3>
               <p class="text-gray-400 text-sm">Ai {{ juristService.readyAlertsCount() }} notificări automate pregătite pentru clienți.</p>
             </div>
           </div>
           <button (click)="sendAllAlerts()" class="w-full md:w-auto bg-green-600 hover:bg-green-500 text-white px-10 py-3 rounded-xl font-black shadow-lg transition-all active:scale-95 text-center">
             TRIMITE TOATE ACUM
           </button>
        </div>
      }
