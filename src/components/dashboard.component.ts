  ...
  template: `
    <div class="h-full flex flex-col space-y-6 overflow-y-auto pb-6 relative animate-fadeIn">
      ...
      @if (juristService.readyAlertsCount() > 0) {
        <div class="bg-gradient-to-br from-green-950/50 via-zinc-900 to-black border border-green-500/50 rounded-2xl p-6 flex flex-col lg:flex-row items-center justify-between gap-8 animate-in fade-in slide-in-from-top-6 duration-700 mb-8 ring-1 ring-green-400/20 shadow-[0_0_50px_rgba(34,197,94,0.15)] relative overflow-hidden group">
           <div class="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(34,197,94,0.1),transparent_70%)] opacity-50"></div>
           
           <div class="flex items-center gap-5 relative z-10">
             <div class="bg-green-500 p-4 rounded-3xl text-black shadow-[0_0_20px_rgba(34,197,94,0.5)] animate-pulse-slow">
             ...
             <div>
               <h3 class="text-white font-black text-xl mb-1 leading-none tracking-tight">Centru de Dispatch WhatsApp</h3>
               <p class="text-gray-400 text-sm max-w-xl leading-relaxed">
                 Ai <span class="text-green-400 font-black">{{ juristService.readyAlertsCount() }}</span> reamintiri pregătite pentru clienți (Termene 24h). 
                 <span class="block mt-2 text-[10px] text-zinc-500 font-bold uppercase tracking-widest bg-zinc-900/80 px-2 py-1 rounded inline-block">Confirmă expedierea calupului prin butonul de mai jos.</span>
               </p>
             </div>
           </div>
           
           <div class="flex flex-col sm:flex-row gap-4 w-full lg:w-auto relative z-10">
             <button (click)="sendAllAlerts()" class="flex-1 lg:flex-none bg-green-600 hover:bg-white hover:text-black text-white px-10 py-4 rounded-2xl text-base font-black transition-all shadow-2xl hover:shadow-[0_0_30px_rgba(255,255,255,0.2)] active:scale-95 flex items-center justify-center gap-3">
               TRIMITE TOATE ACUM
