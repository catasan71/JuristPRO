  ...
  template: `
    <div class="h-full flex flex-col space-y-6 overflow-y-auto pb-6 relative animate-fadeIn">
      ...
      @if (juristService.readyAlertsCount() > 0) {
        <div class="bg-gray-900 border border-green-500/30 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4 animate-slideDown shadow-lg">
           <div class="flex items-center gap-4">
             <div class="bg-green-600 p-3 rounded-full text-white shadow-lg">
             ...
             <div>
               <h3 class="text-white font-bold">Alerte WhatsApp Pregătite</h3>
               <p class="text-gray-400 text-xs">Ai {{ juristService.readyAlertsCount() }} reamintiri de trimis pentru termenele de mâine.</p>
             </div>
           </div>
           <button (click)="sendAllAlerts()" class="bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded-lg text-sm font-bold shadow-lg transition-all active:scale-95">
             Trimite Toate Acum
           </button>
        </div>
      }
