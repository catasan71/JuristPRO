        ...
        <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fadeIn">
          <div class="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-2xl shadow-2xl max-h-[95vh] flex flex-col overflow-hidden">
            ...
            <div class="p-6 border-b border-gray-800 flex justify-between items-center bg-jurist-dark">
              ...
              <button (click)="closeModal()" class="text-gray-400 hover:text-white p-2">✕</button>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input id="caseTitle" [ngModel]="currentEvent.title" (ngModelChange)="updateCurrentEvent('title', $event)" placeholder="Ex: 1234/3/2024" class="w-full bg-black border border-gray-700 rounded-lg p-3 text-white focus:border-jurist-orange outline-none">
                </div>
                <div>
                  <label for="caseType" class="block text-xs font-bold text-gray-400 uppercase mb-1">Tipologie</label>
                  <select id="caseType" [ngModel]="currentEvent.type" (ngModelChange)="updateCurrentEvent('type', $event)" class="w-full bg-black border border-gray-700 rounded-lg p-3 text-white focus:border-jurist-orange outline-none">
                    <option value="court">Instanță</option>
              ...
              <div>
                <label for="caseClient" class="block text-xs font-bold text-gray-400 uppercase mb-1">Client</label>
                <input id="caseClient" [ngModel]="currentEvent.clientName" (ngModelChange)="updateCurrentEvent('clientName', $event)" placeholder="Nume client" class="w-full bg-black border border-gray-700 rounded-lg p-3 text-white focus:border-jurist-orange outline-none">
              </div>

              <div>
                <label for="caseDetails" class="block text-xs font-bold text-gray-400 uppercase mb-1">Instanța / Detalii</label>
                <input id="caseDetails" [ngModel]="currentEvent.details" (ngModelChange)="updateCurrentEvent('details', $event)" placeholder="Ex: Judecătoria Sector 1" class="w-full bg-black border border-gray-700 rounded-lg p-3 text-white focus:border-jurist-orange outline-none">
              </div>

              <div class="bg-gray-800/20 p-5 rounded-xl border border-gray-800">
                <span class="block text-xs font-bold text-gray-500 uppercase mb-4">Financiar (RON)</span>
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    <label for="caseTotal" class="text-[10px] text-gray-400 block mb-1">Onorariu Total</label>
                    <input id="caseTotal" type="number" [ngModel]="currentEvent.financial!.total" (ngModelChange)="updateFinancial('total', $event)" class="w-full bg-black border border-gray-700 rounded p-2 text-white outline-none">
                  </div>
                  <div>
                    <label for="casePaid" class="text-[10px] text-gray-400 block mb-1">Încasat</label>
                    <input id="casePaid" type="number" [ngModel]="currentEvent.financial!.paid" (ngModelChange)="updateFinancial('paid', $event)" class="w-full bg-black border border-gray-700 rounded p-2 text-white outline-none">
                  </div>
                </div>
                <div class="mt-4 pt-3 border-t border-gray-800 flex justify-between items-center text-sm">
                  <span class="text-gray-400">Rest de plată:</span>
                  <span class="font-bold text-jurist-orange">{{ currentEvent.financial!.rest }} RON</span>
                </div>
              </div>
              ...
              <div>
                <div class="flex justify-between items-center mb-2">
                  ...
                  <button (click)="toggleDictation()" [class]="'flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold transition-all ' + (isListening ? 'bg-red-600 text-white animate-pulse' : 'bg-gray-800 text-gray-400')">
                    @if (isListening) {
                ...
                <textarea id="caseNotes" [ngModel]="currentEvent.notes" (ngModelChange)="updateCurrentEvent('notes', $event)" rows="4" class="w-full bg-black border border-gray-700 rounded-lg p-3 text-white focus:border-jurist-orange outline-none resize-none" placeholder="Strategia, probe, martori..."></textarea>
              </div>

              <div class="flex items-center gap-3 bg-gray-800/20 p-4 rounded-xl border border-gray-800">
                <input type="checkbox" id="alert" [ngModel]="currentEvent.whatsappAlert" (ngModelChange)="updateCurrentEvent('whatsappAlert', $event)" [disabled]="!juristService.profile().phone" class="w-5 h-5 accent-jurist-orange">
                <label for="alert" class="text-sm text-gray-300 cursor-pointer">Alertă WhatsApp automată (24h înainte)</label>
              </div>
            </div>

            <div class="p-6 border-t border-gray-800 flex justify-end gap-3 bg-jurist-dark">
              <button (click)="closeModal()" class="px-6 py-2.5 rounded-lg text-gray-400 hover:text-white font-bold">Abandon</button>
              <button (click)="saveEvent()" [disabled]="saving() || !currentEvent.title" class="bg-jurist-orange hover:bg-orange-600 text-black px-10 py-2.5 rounded-lg font-black transition-all disabled:opacity-30 shadow-lg flex items-center justify-center gap-2">
                @if (saving()) {
                  <div class="w-4 h-4 border-2 border-black/20 border-t-black rounded-full animate-spin"></div>
                }
                {{ currentEvent.id ? 'Actualizează' : 'Salvează Dosar' }}
              </button>
            </div>
          </div>
        </div>
